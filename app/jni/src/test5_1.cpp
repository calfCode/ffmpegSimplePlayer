...
SDL_Surface * loadSurface (std::string path){
    ...
    if (!loadedSurface){
        LOGE("Unable to load image %s! SDL Error : %s\\n",path.c_str() , SDL_GetError());
    }
...
bool loadMedia() {
    ...
    gKeyPressSurfaces[KEY_PRESS_SURFACE_DEFAULT] = loadSurface("/data/user/0/org.libsdl.app/app_picture/1.bmp");
    if (!gKeyPressSurfaces[KEY_PRESS_SURFACE_DEFAULT]){
        success = false;
    }

    gKeyPressSurfaces[KEY_PRESS_SURFACE_UP] = loadSurface("/data/user/0/org.libsdl.app/app_picture/up.bmp");
    if (!gKeyPressSurfaces[KEY_PRESS_SURFACE_UP]){
        success = false;
    }

    gKeyPressSurfaces[KEY_PRESS_SURFACE_DOWN] = loadSurface("/data/user/0/org.libsdl.app/app_picture/down.bmp");
    if (!gKeyPressSurfaces[KEY_PRESS_SURFACE_DOWN]){
        success = false;
    }


    gKeyPressSurfaces[KEY_PRESS_SURFACE_LEFT] = loadSurface("/data/user/0/org.libsdl.app/app_picture/left.bmp");
    if (!gKeyPressSurfaces[KEY_PRESS_SURFACE_LEFT]){
        success = false;
    }


    gKeyPressSurfaces[KEY_PRESS_SURFACE_RIGHT] = loadSurface("/data/user/0/org.libsdl.app/app_picture/right.bmp");
            ...
            */
            ...
            SDL_Event event;
                ...
                if (event.type == SDL_KEYDOWN) {
                    LOGD("event.key.keysym.sym=%d", event.key.keysym.sym);
                    switch ( event.key.keysym.sym ){
                        case SDLK_UP:
                            gCurrentSurface = gKeyPressSurfaces[KEY_PRESS_SURFACE_UP];
                            LOGD("SDLK_UP keydown ");
                            break;
                        case SDLK_DOWN:
