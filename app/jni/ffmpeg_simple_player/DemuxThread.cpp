...
int DemuxThread::initDemuxThread()
{
    ...
    int ret = avformat_open_input(&formatCtx, is->filename, NULL, NULL);
    if (ret!= 0) {
        LOGD("avformat_open_input %s Failed:%s",is->filename,strerror(ret));
        return -1;
    }
...
int DemuxThread::decode_loop()
{
    ...
    for(;;) {
        if(m_stop) {
            LOGD("request quit while decode_loop");
            break;
        }
        ...
        if ( ret < 0) {
            LOGD("av_read_frame error:%s",strerror(ret));
            break;
        }
...
int DemuxThread::stream_open(FFmpegPlayerCtx *is, int media_type)
{
    ...
    if (stream_index < 0 || stream_index >= (int)formatCtx->nb_streams) {
        LOGD("Cannot find a audio stream in the input file\n");
        return -1;
    }
    ...
    if (avcodec_open2(codecCtx, codec, NULL) < 0) {
        LOGD("Failed to open codec for stream #%d\n", stream_index);
        return -1;
    }
