...
int FFmpegPlayer::initPlayer()
{
    // init ctx
    playerCtx.init();
    memset(playerCtx.filename,0,1024);
...
void FFmpegPlayer::stop()
{
    ...
    // stop audio decode
    LOGD("audio decode thread clean...");
    if (m_audioDecodeThread) {
        m_audioDecodeThread->stop();
        FREE(m_audioDecodeThread);
    }
    LOGD("audio decode thread finished.");

    // stop audio thread
    LOGD("audio play thread clean...");
    if (m_audioPlay) {
        m_audioPlay->stop();
        FREE(m_audioPlay);
    }
    LOGD("audio device finished.");

    // stop video decode thread
    LOGD("video decode thread clean...");
    if (m_videoDecodeThread) {
        m_videoDecodeThread->stop();
        FREE(m_videoDecodeThread);
    }
    LOGD("video decode thread finished.");

    // stop demux thread
    LOGD("demux thread clean...");
    if (m_demuxThread) {
        m_demuxThread->stop();
        m_demuxThread->finiDemuxThread();
        FREE(m_demuxThread);
    }
    LOGD("demux thread finished.");

    LOGD("player ctx clean...");
    playerCtx.fini();
    LOGD("player ctx finished.");
}
    ...
    switch(e->key.keysym.sym) {
        ...
        do_seek:
            if (true) {
                ...
                LOGD("seek to %lf v:%lf a:%lf", pos, get_audio_clock(&playerCtx), get_audio_clock(&playerCtx));
                stream_seek(&playerCtx, (int64_t)(pos * AV_TIME_BASE), (int)incr);
            }
            break;
        case SDLK_q:
            // do quit
            LOGD("request quit, player will quit");
            ...
            sdlApp->quit();
        case SDLK_SPACE:
            LOGD("request pause, cur state=%d", (int)playerCtx.pause);
